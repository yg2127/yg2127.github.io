---
layout: post
title: "SQLite 1강 - SQLite 기초와 CRUD"
description: "DELETE에 WHERE 안 붙이면 어떻게 될까? — SQL 처음 배울 때 무서웠던 것들"
date: 2026-02-19
category: "SQL"
...
데이터 분석을 하려면 SQL을 알아야 한다고 해서 시작했다. Python만 쓰다가 SQL을 처음 접하니까 문법이 완전히 달라서 어색했다. 특히 "명령형"이 아니라 "선언형"이라서, "어떻게 할지"가 아니라 "뭘 원하는지"를 쓰는 느낌이다. 처음에는 이게 오히려 헷갈렸는데, 익숙해지니까 Python보다 데이터 조회가 훨씬 간결했다.
...
### DELETE/UPDATE에 WHERE를 빼먹으면?

**모든 데이터가 날아가거나 수정된다.** 이건 실수 한 번이면 복구가 안 될 수도 있어서 정말 조심해야 한다. 실무에서는 DELETE/UPDATE 전에 같은 WHERE 조건으로 SELECT를 먼저 해보는 습관을 들이라고 한다.

### DROP vs DELETE 차이

- `DELETE FROM Person` — 데이터만 삭제, 테이블 구조는 남음
- `DROP TABLE Person` — 테이블 자체가 사라짐

### SELECT 실행 순서

SQL은 쓰는 순서와 실행 순서가 다르다: `FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY`. 이 순서를 모르면 WHERE에서 SELECT의 별명을 쓰려다 에러가 나는 등 헷갈리는 상황이 생긴다.

---

## 정리하며

1. SQL은 "뭘 원하는지"를 쓰는 선언형 언어 — Python과 패러다임이 다르다
2. CRUD(Create, Read, Update, Delete)가 SQL의 기본
3. **DELETE/UPDATE에 WHERE 빼먹지 말자** — 가장 흔하고 치명적인 실수
4. GROUP BY + HAVING으로 그룹별 집계와 필터링이 가능하다
