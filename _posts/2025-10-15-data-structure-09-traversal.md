...
series_intro: "노드 다섯 개의 이진트리를 세 방식으로 방문하며 root, subtree와 전위·중위·후위 순회를 익힌다."
series_from: "큐까지는 원소가 한 줄로 이어졌고, 이제 한 노드에서 두 자식으로 갈라지는 계층 구조를 다룬다."
series_to: "이미 만들어진 트리를 방문하는 법을 익힌 뒤 다음 편에서는 입력 관계로 올바른 트리를 만드는 조건을 확인한다."
...
## 갈라지는 구조의 모든 노드를 방문하는 문제

회사 조직도나 폴더 구조처럼 한 시작점에서 아래로 갈라지는 관계를 **트리**로 표현할 수 있다. 트리의 저장 단위는 **노드**, 가장 위의 시작 노드는 **root**, 어떤 노드 아래에 직접 연결된 노드는 **자식**이다. root를 제외한 각 노드는 부모가 하나이고, 연결을 아래로 따라가다가 같은 노드로 돌아오는 순환은 없다. 한 노드와 그 아래의 모든 후손을 묶은 작은 트리를 **subtree**라고 한다.

**이진트리**는 노드마다 왼쪽과 오른쪽 자식을 최대 하나씩 갖는다. 입력으로 이미 만들어진 이진트리의 모든 값을 한 번씩 출력하되, 어떤 문제는 부모를 먼저, 어떤 문제는 자식을 먼저 처리해야 한다. 노드를 빠짐없이 방문하는 절차가 **순회**다.

전위·중위·후위 순회는 왼쪽 subtree와 오른쪽 subtree를 방문하는 순서는 같고, 현재 root를 언제 처리하는지만 다르다. 작은 트리에서 출력 순서를 직접 적으면 세 이름을 따로 외울 필요가 없다.
...
재귀에서 확인한 “돌아온 뒤 남은 문장”을 트리에 적용하면, 그 문장을 왼쪽 호출 전, 두 호출 사이, 오른쪽 호출 뒤 어디에 두는지가 처리 순서를 결정한다.
